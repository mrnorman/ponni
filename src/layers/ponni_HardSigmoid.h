...
namespace ponni {
  ...
  struct HardSigmoid {
    ...
    template <class NewMemorySpace> using rebind_memory_space = HardSigmoid<real,N,NewMemorySpace>;
    ...
    auto copy_to_memory_space(NewMemorySpace const & = NewMemorySpace()) const {
      return rebind_memory_space<NewMemorySpace>(params.num_inputs, params.alpha, params.beta);
    }
