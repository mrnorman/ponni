...
namespace ponni {
  ...
  struct Gelu {
    ...
    doubleHost1d to_array() const {
      doubleHost1d data("GELU_params", 2);
      data(0) = params.num_inputs;
      data(1) = params.approximate ? 1 : 0;
      return data;
    }
    void from_array(doubleHost1d const & data) { init(static_cast<int>(data(0)), data(1) == 1); }
    void validate() const {
      if (params.num_inputs <= 0) Kokkos::abort("ERROR: GELU num_inputs must be > 0");
    }
  };

  template <class real = float, int N = 1> using GELU = Gelu<real,N>;
