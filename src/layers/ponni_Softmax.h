...
namespace ponni {
  ...
  struct Softmax {
    ...
    template <class NewMemorySpace> using rebind_memory_space = Softmax<real,N,NewMemorySpace>;
    ...
    Softmax() = default;
    ~Softmax() = default;
    explicit Softmax(int num_inputs) { init(num_inputs); }
    void init(int num_inputs) { params.num_inputs = num_inputs; }
    char const * get_label() const { return "Softmax"; }
    ...
    KOKKOS_INLINE_FUNCTION static void compute_all_outputs(InputView const & input, OutputView const & output,
      ...
      for (int i = 0; i < p.num_inputs; i++) output(i,ibatch) = std::exp(input(i,ibatch) - max_value) / sum_exp;
    }
    ...
    KOKKOS_INLINE_FUNCTION static void compute_all_outputs(ponni::SArray<real,N> const & input,
      ...
      for (int i = 0; i < N; i++) output(i) = std::exp(input(i) - max_value) / sum_exp;
    }

    void set_trainable_parameters(real1d const &) { }
    real1d get_trainable_parameters() const { return real1d(); }
    doubleHost1d to_array() const {
      doubleHost1d data("Softmax_params", 1); data(0) = params.num_inputs; return data;
    }
    void from_array(doubleHost1d const & data) { init(static_cast<int>(data(0))); }
    void validate() const {
      if (params.num_inputs <= 0) Kokkos::abort("ERROR: Softmax num_inputs must be > 0");
    }
  };
