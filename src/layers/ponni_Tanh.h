...
namespace ponni {
  ...
  struct Tanh {
    ...
    template <class NewMemorySpace> using rebind_memory_space = Tanh<real,N,NewMemorySpace>;
    ...
    Tanh() = default;
    ~Tanh() = default;
    explicit Tanh(int num_inputs) { init(num_inputs); }
    ...
    KOKKOS_INLINE_FUNCTION static real apply(real x) { return std::tanh(x); }
    ...
    doubleHost1d to_array() const { doubleHost1d data("Tanh_params", 1); data(0) = params.num_inputs; return data; }
    void from_array(doubleHost1d const & data) { init(static_cast<int>(data(0))); }
    void validate() const {
      if (params.num_inputs <= 0) Kokkos::abort("ERROR: Tanh num_inputs must be > 0");
    }
  };
