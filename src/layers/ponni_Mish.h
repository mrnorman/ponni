...
namespace ponni {
  ...
  struct Mish {
    ...
    template <class NewMemorySpace> using rebind_memory_space = Mish<real,N,NewMemorySpace>;
    ...
    Mish() = default;
    ~Mish() = default;
    explicit Mish(int num_inputs) { init(num_inputs); }
    void init(int num_inputs) { params.num_inputs = num_inputs; }
    char const * get_label() const { return "Mish"; }
    ...
    KOKKOS_INLINE_FUNCTION static real apply(real x) {
      ...
      if (x > static_cast<real>(0)) {
        softplus = x + std::log(static_cast<real>(1) + std::exp(-x));
      } else {
        softplus = std::log(static_cast<real>(1) + std::exp(x));
      }
    ...
    doubleHost1d to_array() const { doubleHost1d data("Mish_params", 1); data(0) = params.num_inputs; return data; }
    void from_array(doubleHost1d const & data) { init(static_cast<int>(data(0))); }
    void validate() const {
      if (params.num_inputs <= 0) Kokkos::abort("ERROR: Mish num_inputs must be > 0");
    }
  };
