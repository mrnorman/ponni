...
namespace ponni {
  ...
  struct Sigmoid {
    ...
    template <class NewMemorySpace> using rebind_memory_space = Sigmoid<real,N,NewMemorySpace>;
    ...
    Sigmoid() = default;
    ~Sigmoid() = default;
    explicit Sigmoid(int num_inputs) { init(num_inputs); }
    ...
    KOKKOS_INLINE_FUNCTION static real apply(real x) {
      if (x >= static_cast<real>(0)) return static_cast<real>(1) / (static_cast<real>(1) + std::exp(-x));
      real const exp_x = std::exp(x);
      return exp_x / (static_cast<real>(1) + exp_x);
    }
    ...
    doubleHost1d to_array() const {
      doubleHost1d data("Sigmoid_params", 1); data(0) = params.num_inputs; return data;
    }
    void from_array(doubleHost1d const & data) { init(static_cast<int>(data(0))); }
    void validate() const {
      if (params.num_inputs <= 0) Kokkos::abort("ERROR: Sigmoid num_inputs must be > 0");
    }
  };
