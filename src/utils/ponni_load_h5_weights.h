...
namespace ponni {
  ...
  load_h5_weights( std::string file_name    ,
    ...
    typedef typename yakl::Array<typename yakl::ViewType<float,N>::type> ARR_TYPE;
    ...
    if ( N == 1 || (! transpose) ) {
      ...
      if        constexpr (N == 2) {
        ARR_TYPE("weights_transposed",dims[1],dims[0]);
        ...
        ARR_TYPE("weights_transposed",dims[2],dims[1],dims[0]);
        ...
        ARR_TYPE("weights_transposed",dims[3],dims[2],dims[1],dims[0]);
