...
namespace ponni {
  ...
  load_h5_weights( std::string file_name    ,
    ...
    ARR_HOST weights_host;
    if      constexpr (N == 1) { weights_host = ARR_HOST("weights_host",dims[0]);                         }
    else if constexpr (N == 2) { weights_host = ARR_HOST("weights_host",dims[0],dims[1]);                 }
    else if constexpr (N == 3) { weights_host = ARR_HOST("weights_host",dims[0],dims[1],dims[2]);         }
    else if constexpr (N == 4) { weights_host = ARR_HOST("weights_host",dims[0],dims[1],dims[2],dims[3]); }
