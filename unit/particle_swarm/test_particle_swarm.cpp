...
int main( int argc , char **argv ) {
  ...
  yakl::init();
  {
    ...
    for (int iter = 0; iter < num_iterations; iter++) {
      auto ensemble = pso.get_ensemble();
      auto params   = ensemble.get_parameters();
      auto loss     = ensemble.get_loss();

      parallel_for( YAKL_AUTO_LABEL() , ensemble.get_ensemble_size() , YAKL_LAMBDA (int iens) {
        loss(iens) = func(params(0,iens),params(1,iens));
      });

      pso.update_from_ensemble( ensemble );
    }
