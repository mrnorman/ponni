...
if [[ -x "${venv_python}" ]]; then
  ...
  if [[ "${have_version}" == "${want_version}" ]] && "${venv_python}" -c 'import torch, keras, h5py, numpy' >/dev/null 2>&1; then
    env_is_valid=true
  fi
fi
...
if [[ "${env_is_valid}" != "true" ]]; then
  ...
  "${uv_bin}" pip install --python "${venv_python}" numpy h5py torch keras
fi
